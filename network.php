...
<body>
  ...
  <main class="container py-4">
    ...
    <div class="row g-4">
      ...
      <!-- Left Panel: Conversations, Connections & Pending -->
      <div class="col-lg-4 network-sidebar" id="networkSidebar">
        ...
        <!-- Tabbed Panel: Messages / My Connections -->
        <div class="konekt-card no-hover-lift p-3">
          <div class="network-tabs">
            <button class="network-tab active" data-tab="messages" id="tabMessages">
              Messages
              <?php if ($unread_messages > 0): ?>
                <span class="tab-count"><?= $unread_messages ?></span>
              <?php endif; ?>
            </button>
            <button class="network-tab" data-tab="connections" id="tabConnections">
              Connections
              <span class="tab-count"><?= count($myConnections) ?></span>
            </button>
          </div>

          <!-- Tab: Messages -->
          <div class="network-tab-content active" id="tabContentMessages">
          ...
          <!-- Tab: My Connections -->
          <div class="network-tab-content" id="tabContentConnections">
            <?php if (empty($myConnections)): ?>
              <p class="text-secondary small mb-0">No connections yet. Search for people and send connection requests.</p>
            <?php else: ?>
              <?php foreach ($myConnections as $conn): ?>
              <a href="network.php?user_id=<?= $conn['user_id'] ?>" class="connection-item">
                <div class="applicant-avatar" style="width:36px; height:36px; font-size: 0.75rem;">
                  <?= getInitials($conn['first_name'], $conn['last_name']) ?>
                </div>
                <div class="connection-info">
                  <p class="connection-name"><?= htmlspecialchars($conn['first_name'] . ' ' . $conn['last_name']) ?></p>
                  <p class="connection-headline"><?= htmlspecialchars($conn['headline'] ?? 'KoneKT User') ?></p>
                </div>
                <span class="btn btn-konekt-primary btn-sm"><i class="bi bi-chat-dots-fill"></i></span>
              </a>
              <?php endforeach; ?>
            <?php endif; ?>
          </div>
      ...
      <div class="col-lg-8 network-chat-panel <?= $activeChatUserId ? 'mobile-active' : '' ?>" id="networkChatPanel">
